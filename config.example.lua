...
-- 当前硬件不使用外置存储和录音上传，电话流程不会依赖录音文件或 UPLOAD_URL

-------------------------------------------------- 短信来电配置 --------------------------------------------------

-- 允许发短信控制设备的号码, 如果注释掉或者为空, 则允许所有号码, 短信格式示例:
-- 拨打电话并播放默认 TTS CALL,10086
-- 发送短信 SMS,10086,查询流量
-- 查询所有呼转状态 CCFC,?
...
-- 来电自动接听后播放的 TTS 语音内容；未配置或为空时，自动接听后直接挂断
-- TTS_TEXT = "您好，这是一条电话通知。"

-- 来电动作, 0:无操作, 1:自动接听后播放 TTS/直接挂断, 2:挂断, 3:自动接听后播放 TTS/直接挂断, 4:等待30秒后自动接听
